    ...
    const fetchData = async () => {
      ...
      try {
        const res = await loadingFetch(
          `${API}${Constants.ROUTES.ANALYTICS_COMPLAINTS}?period=${period}&date=${dateValue}&status=${status}`,
          { credentials: "include" }
        )

        if (!res.ok) {
          showAlert("Could not fetch complaints analytics!", "error")
          return
        }
        ...
        showAlert("Failed to load complaints analytics!", "error")
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className="analytics-details">
      ...
      <div className="stats-summary">
        ...
        <div className="summary-card"><h3>In Progress</h3><p>{data?.progress || 0}</p></div>
      </div>

      <div className="period-selector" style={{marginBottom: "20px"}}>
        {["all", "pending", "resolved", "rejected", "in_progress"].map(s => (
