    ...
    const fetchData = async () => {
      ...
      try {
        const res = await loadingFetch(
          `${API}${Constants.ROUTES.ANALYTICS_ROOM_CHANGE}?period=${period}&date=${dateValue}&status=${status}`,
          { credentials: "include" }
        )

        if (!res.ok) {
          showAlert("Could not fetch room_change analytics!", "error")
          return
        }
        ...
        showAlert("Failed to load room_change analytics!", "error")
      } finally {
        setLoading(false)
      }
    }
...
export default RoomChangeAnalytics