...
function MealRequestsAnalytics() {
    ...
    const fetchData = async () => {
      ...
      try {
        const res = await loadingFetch(
          `${API}${Constants.ROUTES.ANALYTICS_MEAL_REQUESTS}?period=${period}&date=${dateValue}&status=${status}`,
          { credentials: "include" }
        )

        if (!res.ok) {
          showAlert("Could not fetch meal_requests analytics!", "error")
          return
        }
        ...
        showAlert("Failed to load meal_requests analytics!", "error")
      } finally {
        setLoading(false)
      }
    }
...
export default MealRequestsAnalytics